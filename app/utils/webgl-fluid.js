 ...
 * Refactored for Vue/Nuxt Single Page Applications.
 * Encapsulates state, accepts config/canvas via parameters, and returns a strict cleanup function.
 */

export default function initFluid(canvas, userConfig) {
  // 1. Properly merge default settings with any user overrides
  const config = {
    SIM_RESOLUTION: 128,
    DYE_RESOLUTION: 1024,
  ...
  // Note: Ensure this image is accessible in your Nuxt public folder (e.g., /LDR_LLL1_0.png)
  ...
  function generateColor() {
    let c = config.RANDOM_COLORS
      ? HSVtoRGB(Math.random(), 1.0, 1.0)
      : HSVtoRGB(config.SPLAT_HUE, 1.0, 1.0);
    c.r *= 0.15;
    c.g *= 0.15;
    c.b *= 0.15;
    return c;
